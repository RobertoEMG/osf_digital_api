...
async function MsgMasivo(req, res) {
    const { titulo, descrip, identity, force_logout } = req.body;
        ...
        if (!err) {
            ...
            for(var i=0; i<dtsJson.length; i++){
                sendMessaje(
                    ...
                    dtsJson[i].token, titulo, descrip, identity, force_logout,
                    //"-- AVISO IMPORTANTE --",
                    //"Versión 1.0.9",
                    ...
                    //"La actualización ya está lista en el Google Play para su descarga, cuaquier duda nos pueden llamar en horario habil...",
